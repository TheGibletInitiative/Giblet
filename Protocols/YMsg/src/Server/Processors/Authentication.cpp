...
namespace Giblet { namespace Protocols { namespace YMsg { namespace Server { namespace Processors
{
	...
	void Authentication::Process(session_type& session, const header_type& header, payload_type& payload)
	{
		...
		auto& profileManager(session.GetProfileManager());
		profileManager.Load(session.GetClientId(), { "winky", "dinky", "blinky", "brad", "greg" });

		auto& contactManager(session.GetContactManager());
		contactManager.LoadContact("Friends", ContactInfo(ContactInfo::Linked, "jay"));
		contactManager.LoadContact("Friends", ContactInfo(ContactInfo::Linked, "donna", ContactInfo::availability_type::Custom, false, "Listening to Bewiz - Booters Paradise"));
		contactManager.LoadContact("Friends", ContactInfo(ContactInfo::Linked, "david", ContactInfo::availability_type::Busy, true));
		contactManager.LoadContact("Myself", ContactInfo(ContactInfo::Linked, "winky", initialAvailability));
		contactManager.LoadContact("Myself", ContactInfo(ContactInfo::Linked, "dinky", initialAvailability));
		contactManager.LoadContact("Myself", ContactInfo(ContactInfo::Linked, "blinky", initialAvailability));
		contactManager.LoadContact("Myself", ContactInfo(ContactInfo::Linked, "brad", initialAvailability));

		auto& blockedContactManager(session.GetBlockedContactManager());
...

		{
			...
			clientProfileBuilder.Build(session.GetConnection(), session);
			clientProfileBuilder.Send(session.GetConnection());
		}

		{
			...
			copy_if(contactManager.begin(), contactManager.end(), back_inserter(onlineContacts), isContactOnline);

			Builders::SetContactStatus contactOnlineBuilder;
			contactOnlineBuilder.Build(session.GetConnection(), payload.clientId1_, onlineContacts);
			contactOnlineBuilder.Send(session.GetConnection());
		}
...

		{
			...
			pingConfigBuilder.Build(session.GetConnection());
			pingConfigBuilder.Send(session.GetConnection());
		}
		...
		if(session.GetClientId().find('@') != string_type::npos)
		{
			...
			yahooAlsertBuilder.Send(session.GetConnection());
		}

		if(session.GetClientId().find('-') != string_type::npos)
		{
			Builders::NewMailNotification newMailNotifyBuilder;
			newMailNotifyBuilder.Build(session.GetConnection(), 5);
			newMailNotifyBuilder.Send(session.GetConnection());
		}

		if(session.GetClientId().find('_') != string_type::npos)
		{
			Builders::NewMailNotification newMailNotifyBuilder;
			newMailNotifyBuilder.Build(session.GetConnection(), "me@yahoo.com", "Some Guy", "Some subject");
			newMailNotifyBuilder.Send(session.GetConnection());
		}
		...
		if(session.GetClientId().find('.') != string_type::npos)
		{
			...
			messageBuilder.Build(session.GetConnection(), session.GetClientId(), "Ed_", "Hello!", Builders::OfflineMessage::encoding_type::None);
			messageBuilder.Send(session.GetConnection());
		}
